...
export const ArtifactCard: React.FC<ArtifactCardProps> = ({ artifact, variant = 'default' }) => {
  ...
  const primaryImage = imageChain[0] || '';
  const hasImage = !!primaryImage;

  if (variant === 'compact') {
    return (
      <CompactCard artifact={artifact} imageChain={imageChain} hasImage={hasImage} />
    );
  }

  return (
    <DefaultCard artifact={artifact} imageChain={imageChain} hasImage={hasImage} />
  );
};
              ...
              <AuthImage
                ...
                fallback={
                  <div className="w-full h-full flex items-center justify-center bg-gray-100">
                    <Gem className="h-6 w-6 text-museum-gold/30 animate-pulse" />
                  </div>
                }
                errorFallback={
            ...
            <AuthImage
              ...
              fallback={
                <div className="w-full h-full flex items-center justify-center bg-gradient-to-br from-gray-50 to-gray-100">
                  <Gem className="h-12 w-12 text-museum-gold/20 animate-pulse" />
                </div>
              }
              errorFallback={
