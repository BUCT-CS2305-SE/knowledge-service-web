...
import { resolveImageUrl, getFallbackImageChain } from '@/utils/imageUtils';
...
export const DetailPage: React.FC = () => {
  ...
  const [mainImageFallbackIdx, setMainImageFallbackIdx] = useState<number>(0);
  ...
  // 切换图片或文物时重置降级链状态
  useEffect(() => {
    setMainImageFallbackIdx(0);
    setMainImageFailed(false);
  }, [id, selectedImageIndex]);

  // 构建主图降级链
  const mainImageChain = artifact
    ? getFallbackImageChain(artifact.images, artifact.id)
    : [];
  const currentMainImage = mainImageChain[mainImageFallbackIdx] || '';
  const handleMainImageError = (): void => {
    const nextIdx = mainImageFallbackIdx + 1;
    if (nextIdx < mainImageChain.length) {
      setMainImageFallbackIdx(nextIdx);
    } else {
  ...
  const handleCollectToggle = async (): Promise<void> => {
    if (!id || !artifact) return;
    if (!isAuthenticated) {
      // 未登录，跳转到登录页
              ...
              <AuthImage
                ...
                fallback={
                  <div className="w-full h-full flex items-center justify-center bg-gradient-to-br from-museum-gold/5 to-museum-gold/10">
                    <Gem className="h-16 w-16 text-museum-gold/20 animate-pulse" />
                  </div>
                }
                errorFallback={
                  <div className="w-full h-full flex items-center justify-center bg-gradient-to-br from-museum-gold/5 to-museum-gold/10">
          ...
          {artifact.images.length > 1 && !imageZoomed && (
            <div className="flex gap-3 overflow-x-auto pb-2">
              {artifact.images.map((image, index) => (
                ...
                >
                  <AuthImage
                    src={resolveImageUrl(image, { width: 200 })}
                    alt={`${artifact.name} 缩略图 ${index + 1}`}
                    className="w-full h-full object-cover"
                    errorFallback={
                      <div className="w-full h-full flex items-center justify-center bg-gray-100">
                        <Gem className="h-4 w-4 text-gray-300" />
                      </div>
                    }
                  />
                </button>
        ...
        <div className="space-y-6">
          ...
          <div className="flex flex-wrap gap-3">
            ...
            {artifact.detailUrl && (
              <Button
                size="sm"
                className="gap-2 bg-museum-gold hover:bg-museum-gold-dark"
                onClick={() => window.open(artifact.detailUrl, '_blank')}
              >
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-2">
              {artifact.tags.map(tag => (
                <Link to={`/browse?search=${encodeURIComponent(tag)}`} key={tag}>
                  <Badge
                    variant="outline"
