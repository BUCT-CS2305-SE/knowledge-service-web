...
export const HomePage: React.FC = () => {
  ...
  // 个性化推荐：基于浏览记录和收藏记录（匹配年代/时期和类别）
  useEffect(() => {
    const loadRecommendations = async (): Promise<void> => {
      try {
        setLoadingRecommendations(true);

        // 收集用户兴趣：提取朝代关键词用于模糊匹配
        const interestKeywords: string[] = [];
        const seenIds = new Set<string>();

        // Known dynasty names for extraction from era strings
        const DYNASTY_NAMES = ['Qing', 'Ming', 'Tang', 'Song', 'Han', 'Yuan', 'Zhou', 'Shang', 'Qin',
                               '清朝', '明朝', '唐朝', '宋朝', '汉朝', '元朝', '周朝', '商朝', '秦朝'];

        browseHistory.forEach((h) => {
          seenIds.add(h.artifactId);
          if (h.artifactEra) {
          ...
          if (h.artifactCategory && h.artifactCategory !== '其他' && h.artifactCategory !== '未知') {
        ...
        collectionItems.forEach((item) => {
          ...
          if (item.artifactEra) {
            for (const name of DYNASTY_NAMES) {
              if (item.artifactEra.includes(name) && !interestKeywords.includes(name)) {
                interestKeywords.push(name);
              }
            }
          }
          if (item.artifactCategory && item.artifactCategory !== '其他' && item.artifactCategory !== '未知') {
            if (!interestKeywords.includes(item.artifactCategory)) {
              interestKeywords.push(item.artifactCategory);
            }
          }
        });

        // 获取文物进行匹配
        const response = await artifactService.getArtifacts({ page: 1, size: 50 });
        const allArtifacts = response.data;

        let recommendations: typeof allArtifacts = [];

        if (interestKeywords.length > 0) {
          // 有用户兴趣数据：模糊匹配（era字段包含朝代关键词即可）
          recommendations = allArtifacts
            .filter((artifact) => !seenIds.has(artifact.id))
            .filter((artifact) => {
              return interestKeywords.some(kw =>
                artifact.era.includes(kw) || artifact.category.includes(kw)
              );
            })
            .slice(0, 4);
        }
        ...
        if (recommendations.length < 4) {
          const remaining = allArtifacts
            .filter(a => !seenIds.has(a.id) && !recommendations.find(r => r.id === a.id))
        ...
        <div className="museum-container">
          <div className="flex justify-between items-end mb-12">
            <div>
              ...
              <p className="text-lg text-gray-600">
                {isAuthenticated && (browseHistory.length > 0 || collectionItems.length > 0)
