...
const SearchPage: React.FC = () => {
  ...
  const doSearch = async (kw: string, page: number = 1) => {
    ...
    try {
      const response = await artifactService.searchArtifacts(kw.trim(), page, PAGE_SIZE);
        ...
        <>
          ...
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
            <p className="text-gray-600">
              找到 <span className="font-semibold text-museum-gold-dark">{total}</span> 件相关文物
              {totalPages > 1 && <span className="text-gray-400">（第 {currentPage}/{totalPages} 页）</span>}
            </p>
              ...
              <div className="flex items-center gap-2">
                {Array.from({ length: Math.min(totalPages, 7) }).map((_, i) => {
                  ...
                  if (totalPages <= 7) {
                    ...
                    pageNum = i < 3 ? i + 1 : i === 3 ? currentPage - 1 : i === 4 ? currentPage : totalPages;
                  }
                  return (
                    <Button
                      key={pageNum}
