  ...
  return (
    <div className="museum-container py-8 md:py-12">
      ...
      <div className="flex flex-col lg:flex-row gap-8">
        ...
        {/* Mobile Filter Panel */}
        {mobileFilterOpen && (
          <div className="lg:hidden fixed inset-0 z-50 bg-white p-4 overflow-auto">
            <div className="flex justify-between items-center mb-4">
              <h3 className="font-bold text-lg">筛选条件</h3>
              <Button variant="ghost" size="sm" onClick={() => setMobileFilterOpen(false)}>✕ 关闭</Button>
            </div>
            <FilterPanel />
          </div>
        )}
          ...
          <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-4 mb-6">
            <div className="flex flex-col lg:flex-row gap-4 items-start lg:items-center justify-between">
              ...
              <div className="flex items-center gap-4 w-full lg:w-auto justify-between lg:justify-end">
                ...
                <Link to={compareList.length > 0 ? "/compare" : "#"}>
                  <Button
                    size="sm"
                    disabled={compareList.length === 0}
                    className="bg-museum-gold hover:bg-museum-gold-dark text-white disabled:opacity-50 disabled:cursor-not-allowed"
                    title={compareList.length === 0 ? "请先在文物列表中添加文物进行对比" : "查看对比"}
                  >
                    对比{compareList.length > 0 ? ` (${compareList.length})` : ''}
                  </Button>
                </Link>
              </div>
            </div>
