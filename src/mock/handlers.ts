...
export const mockApi = {
  getArtifacts: async (params: FilterParams): Promise<PaginatedResponse<Artifact>> => {
    ...
    if (params.sortBy) {
      if (params.sortBy === 'updated') {
        ...
        const sortKey = params.sortBy as keyof Artifact;
        filtered.sort((a, b) => {
  ...
  getFilterOptions: async (): Promise<FilterOptions> => {
    ...
    // 从 artifacts 动态提取 eras（支持中文朝代格式）
    const eraSet = new Set<string>();
    artifacts.forEach(a => {
      if (a.era) {
        // 提取中文朝代关键词（如 "清朝 (1644–1911)" → "清朝"）
        const zhMatch = a.era.match(/([^\s]+朝)/);
        if (zhMatch) {
          eraSet.add(zhMatch[1]);
        }
        // 兼容英文朝代格式（如 "Qing dynasty"）
        const enMatch = a.era.match(/([A-Z][a-z]+)\s+(dynasty|Dynasty)/);
        if (enMatch) {
          eraSet.add(enMatch[1] + ' Dynasty');
        }
      }
    });

    const eras = Array.from(eraSet).sort();
    ...
    return {
      ...
      materials,
      museums,
      eras: eras.map(e => ({ value: e, label: e })),
    };
  },
...
export const userApi = {
  ...
  addCollectionItem: async (
    ...
    const existing = items.find(
      (item) => item.artifactId === artifactId && item.groupId === groupId
    );
    ...
    items.push(newItem);
    collectionItemsStore.set(userId, items);
    return newItem;
  },
   ...
   * 移除收藏项
   */
  removeCollectionItem: async (itemId: string): Promise<void> => {
    await delay(Math.random() * 50 + 20);

    for (const [userId, items] of collectionItemsStore) {
      const filtered = items.filter((item) => item.id !== itemId);
      if (filtered.length !== items.length) {
        collectionItemsStore.set(userId, filtered);
        break;
      }
    }
  },
  ...
  moveCollectionItem: async (itemId: string, targetGroupId: string): Promise<void> => {
    ...
    for (const [, items] of collectionItemsStore) {
