...
- **📊 数据可视化模块** ← 新增
  - 统计分析看板：文物总数、类型占比、朝代分布、博物馆藏量排行
  - 知识图谱关系图：力导向图展示文物实体关联关系
  - 文物时间轴：按朝代/自定义年份筛选，点击跳转文物列表
  - 文物地理分布图：世界地图展示博物馆位置，支持点击查看藏品详情
---
...
│   ├── Statistics.tsx        # 统计分析看板（饼图/柱状图/总览卡片）
│   ├── KnowledgeGraph.tsx    # 知识图谱关系图（力导向图）
│   ├── Timeline.tsx          # 文物时间轴（折线图/朝代/年份筛选）
│   └── Map.tsx               # 文物地理分布图（Leaflet世界地图）
...
#### 统计分析看板 (`/statistics`)
- 顶部总览卡片：文物总数、博物馆数量、文物类型数、历史朝代数
- 文物类型占比饼图（陶瓷、书画、青铜器等）
- 博物馆藏量排行饼图（各博物馆藏品占比）
- 各朝代文物数量分布柱状图

#### 知识图谱关系图 (`/knowledge-graph`)
- 力导向图布局，支持节点拖拽、缩放
- 悬停显示节点属性（名称、类型）
- 点击节点在右侧显示详情面板
- 边线带箭头和关系标签
...
- 折线图/柱状图双视图切换
- 支持按朝代下拉筛选
- 支持自定义年份范围筛选（起始年份～结束年份）
- 点击数据点跳转到对应朝代的文物列表
- 底部表格展示朝代详细信息

#### 文物地理分布图 (`/map`)
- 基于 Leaflet + OpenStreetMap 的世界地图底板
- 红色标记展示博物馆地理位置
- 圆圈大小代表藏品数量
- 点击标记弹出博物馆详情弹窗
- 右侧统计摘要和博物馆列表联动
...
| **Leaflet** | 1.9.4 | 地图可视化（世界地图）|
| **react-leaflet** | 4.2.1 | React 地图组件封装 |
| **echarts** | 5.5.0 | 图表可视化（饼图、柱状图、力导向图）|
| **echarts-for-react** | latest | ECharts React 封装 |
---
